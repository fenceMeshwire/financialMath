...
def calc_VAT(vat_factor, gross_price):
    vat_vactor_drinks:float = 1.19 # Value added tax for drinks
    vat_factor_food:float = 1.07 # Value added tax for drinks
    net_price_drinks:float = 0
    net_price_food:float = 0
    vat_drinks:float = 0
    vat_food:float = 0
    if vat_factor == 19:
...
for article in articles:
    ...
    nets_drinks += net_price_drinks
    nets_foods += net_price_food
    vats_drinks += vat_drinks
    vats_foods += vat_food
...
gross_price_food = format(grp_food, '0.2f')
gross_price_drinks = format(grp_drinks, '0.2f')
nets_drinks = format(nets_drinks, '0.2f')
nets_foods = format(nets_foods, '0.2f')
vats_drinks = format(vats_drinks, '0.2f')
vats_foods = format(vats_foods, '0.2f')

# Print the output
print('Gross sum all articles:', sum_gross)
print('Gross price food:', gross_price_food)
print('Gross price drinks:', gross_price_drinks)
print('Value added tax for drinks:', vats_drinks)
print('Value added tax for food:', vats_foods)
print('Net price for drinks:', nets_drinks)
print('Net price for food:', nets_foods)
